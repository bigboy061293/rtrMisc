...
while True:
	...
	if (abc[0] == 'H'):
		...
		av = [float(abc[0]) , float(abc[1]), float(abc[2]) ,float(abc[3]) ,float(abc[4])]
		points = np.vstack([points, av])
		...
		#del tempLatX[:]
		#del tempLongY[:]
		#pointCloudPoints = len(points[:,1])
		#pointCloudPoints = len(points)
		#print pointCloudPoints
		#if pointCloudPoints > 5:
		#	for i in range(pointCloudPoints - 1):
